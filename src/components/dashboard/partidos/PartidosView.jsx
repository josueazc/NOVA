      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="mb-20 flex flex-col lg:flex-row gap-16 items-center justify-between">
           ...
           <div className="w-full lg:w-1/2 space-y-8">
              <div className="inline-flex items-center gap-2 bg-surface text-accent px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest border border-line shadow-sm transition-colors">
              ...
              <h1 className="text-5xl sm:text-6xl font-black tracking-tighter leading-[0.9] text-ink">
                Mapa de <br />
                <span className="text-accent transition-colors">Agrupaciones</span>
              </h1>
              
              <p className="text-muted text-xl font-medium leading-relaxed max-w-lg transition-colors">
              ...
              <div className="relative group w-full max-w-md mt-4">
                 <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-faint group-focus-within:text-accent transition-colors" size={24} />
                 <input 
                   type="text" 
                   placeholder="Escribe un partido o siglas..." 
                   value={searchTerm}
                   onChange={(e) => setSearchTerm(e.target.value)}
                   className="bg-surface  border-none rounded-full py-5 pl-16 pr-6 text-base font-bold outline-none focus:ring-4 focus:ring-accent/15 w-full transition-all shadow-card"
                 />
              </div>
           </div>
           ...
           <div className="w-full lg:w-1/2">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                 ...
                 <div className="bg-surface p-8 rounded-3xl shadow-card border border-line flex flex-col justify-center sm:col-span-2 relative overflow-hidden group transition-colors">
                   <div className="absolute right-0 top-0 opacity-[0.03] group-hover:scale-125 transition-transform duration-700">
                     <Shield size={200} />
                   </div>
                   <h4 className="text-faint font-bold uppercase tracking-widest text-[10px] mb-2 relative z-10">Total Inscritos</h4>
                   <div className="flex items-end gap-3 relative z-10">
                     <span className="text-6xl lg:text-7xl font-black text-accent tracking-tighter leading-none transition-colors">{partidos.length}</span>
                     <span className="text-danger font-black uppercase text-sm mb-2">Oficiales</span>
                   </div>
                 </div>
        ...
        <div className="flex justify-between items-center mb-10 px-2">
           <h4 className="text-[12px] font-black text-faint uppercase tracking-[0.3em]">Resultados: {processedPartidos.length}</h4>
           <button 
             onClick={() => setSortBy(sortBy === 'asc' ? 'desc' : 'asc')}
             className="bg-surface p-3 rounded-xl shadow-sm text-accent flex items-center gap-2 border border-line hover:shadow-md transition-all active:scale-95"
           >
...
const PartyCard = ({ party, onClick }) => (
  ...
  >
    ...
    <div className="px-2 flex flex-col items-center text-center flex-grow relative z-10 justify-center">
      <div className="bg-surface-2 px-5 py-2 rounded-xl group-hover:bg-accent group-hover:text-white transition-colors duration-500 mb-4 shadow-sm">
      ...
      <h3 className="text-2xl font-black tracking-tighter text-ink leading-[1.1] group-hover:text-accent transition-colors duration-300">
        {party.nombre}
      </h3>
    </div>
    ...
    <div className="absolute bottom-6 right-6 opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-4 group-hover:translate-y-0 z-20">
      <div className="bg-danger text-white w-14 h-14 rounded-full shadow-2xl flex items-center justify-center hover:bg-red-700 hover:scale-110 transition-all cursor-pointer">
...
const InfoTile = ({ label, value, icon }) => (
  <div className="bg-surface p-8 rounded-3xl border border-line flex flex-col items-center justify-center text-center shadow-card transition-colors">
    <div className="text-faint mb-4">{icon}</div>
    <div className="text-3xl font-black text-accent tracking-tighter mb-1 transition-colors">{value}</div>
    <div className="text-[10px] font-black text-faint uppercase tracking-widest leading-none">{label}</div>
  </div>
);
