...
"""`;
 ...
 return (
  <div className="flex-grow flex flex-col w-full animate-in fade-in duration-500 pb-10 transition-colors">
   ...
   {/* Header del Tab */}
   <div className="bg-accent text-white py-12 px-6 lg:px-24 w-full rounded-b-[3rem] shadow-xl relative overflow-hidden shrink-0 transition-colors">
    ...
    <div className="max-w-[1600px] mx-auto flex flex-col gap-4 relative z-10">
     <div className="flex items-center gap-3">
      ...
      <h1 className="text-4xl font-black tracking-tighter">Asistente IA Estricto</h1>
     </div>
   ...
   <div className="flex-grow flex flex-col lg:flex-row gap-8 max-w-[1600px] w-full mx-auto px-6 lg:px-24 mt-8 h-[700px]">
    ...
    <div className="w-full lg:w-1/3 flex flex-col bg-surface rounded-2xl shadow-xl border border-line overflow-hidden shrink-0 transition-colors">
     <div className="bg-surface-2 p-6 border-b border-line flex items-center justify-between transition-colors">
      <div className="flex items-center gap-3">
       <div className="w-10 h-10 bg-accent text-white rounded-xl flex items-center justify-center shadow-md transition-colors">
        <Database size={18} />
       </div>
       <div>
        <h3 className="font-black text-ink tracking-tight transition-colors">Base de Conocimiento</h3>
        <p className="text-[10px] text-muted uppercase font-bold tracking-widest transition-colors">Alimenta a la IA</p>
       </div>
      </div>
      ...
      <button
       ...
       className="bg-danger hover:bg-red-700 text-white px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest flex items-center gap-2 transition-all shadow-md active:scale-95"
      >
     ...
     <div className="p-6 flex-grow flex flex-col gap-4">
      ...
      {files.length > 0 && (
       <div className="flex flex-wrap gap-2 mb-2">
        {files.map((f, i) => (
         <div key={i} className="flex items-center gap-2 bg-blue-50 border border-blue-100 dark:border-blue-800 text-blue-800 dark:text-blue-200 px-3 py-1.5 rounded-lg text-xs font-bold shadow-sm transition-colors">
      ...
      <textarea
       ...
       className="flex-grow w-full bg-surface-2 border-2 border-line rounded-2xl p-5 text-sm outline-none focus:border-accent transition-colors resize-none font-medium text-ink"
      ></textarea>
     </div>
    </div>

    {/* COLUMNA DERECHA: CHAT */}
    <div className="w-full lg:w-2/3 flex flex-col bg-surface rounded-2xl shadow-xl border border-line overflow-hidden transition-colors">
     <div className="bg-surface-2 p-6 border-b border-line flex items-center justify-between transition-colors">
      <div className="flex items-center gap-3">
       <div className="w-10 h-10 bg-blue-100 dark:bg-blue-900/50 text-blue-600 rounded-xl flex items-center justify-center shadow-sm transition-colors">
        <Bot size={20} />
       </div>
       <div>
        <h3 className="font-black text-ink tracking-tight transition-colors">Chat Interactivo</h3>
        <p className="text-[10px] text-muted uppercase font-bold tracking-widest transition-colors">Soporta: Español, Inglés, etc.</p>
       </div>
      </div>
     </div>
     ...
     <div className="flex-grow overflow-y-auto p-6 space-y-6 bg-slate-50/50 /50 transition-colors">
      {messages.map(msg => (
       <div key={msg.id} className={`flex gap-4 ${msg.sender === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        <div className={`px-5 py-4 rounded-2xl max-w-[80%] text-sm leading-relaxed shadow-sm transition-colors ${msg.sender === 'user'
      ...
      {isLoading && (
       <div className="flex gap-4">
        <div className="w-10 h-10 rounded-2xl bg-accent text-white flex items-center justify-center flex-shrink-0 mt-1 shadow-md transition-colors">
         <Bot size={18} />
        </div>
        <div className="px-5 py-4 rounded-2xl bg-white border border-line rounded-tl-sm flex items-center gap-3 shadow-sm transition-colors">
         <Loader2 size={18} className="text-accent animate-spin transition-colors" />
         <span className="text-xs text-muted font-bold tracking-wider uppercase transition-colors">Analizando documentos...</span>
        </div>
       </div>
      )}
     </div>

     <div className="p-5 bg-surface border-t border-line transition-colors">
      <form onSubmit={handleSend} className="relative flex items-center">
       <input
        ...
        className="w-full bg-surface-2 border-2 border-line rounded-2xl pl-6 pr-16 py-4 text-sm outline-none focus:border-accent transition-all font-medium text-ink"
        disabled={isLoading}
       />
       <button
        type="submit"
        disabled={!input.trim() || isLoading}
        className="absolute right-3 w-12 h-12 bg-accent hover:bg-blue-800 dark:hover:bg-blue-700 disabled:opacity-50 text-white rounded-[1rem] flex items-center justify-center transition-all shadow-lg active:scale-95"
       >
