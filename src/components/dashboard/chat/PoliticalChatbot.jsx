...
"""`;
  ...
  return (
    <div className="flex-grow flex flex-col bg-slate-50 w-full animate-in fade-in duration-500 pb-10">

      {/* Header del Tab */}
      <div className="bg-[#001D4A] text-white py-12 px-6 lg:px-24 w-full rounded-b-[3rem] shadow-xl relative overflow-hidden shrink-0">
      ...
      <div className="flex-grow flex flex-col lg:flex-row gap-8 max-w-[1600px] w-full mx-auto px-6 lg:px-24 mt-8 h-[700px]">
        ...
        <div className="w-full lg:w-1/3 flex flex-col bg-white rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden shrink-0">
          <div className="bg-slate-100 p-6 border-b border-slate-200 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-[#001D4A] text-white rounded-xl flex items-center justify-center shadow-md">
                <Database size={18} />
              </div>
              <div>
                <h3 className="font-black text-slate-800 tracking-tight">Base de Conocimiento</h3>
                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">Alimenta a la IA</p>
              </div>
            </div>
          ...
          <div className="p-6 flex-grow flex flex-col gap-4">
            <div className="flex gap-3 items-start bg-amber-50 p-4 rounded-2xl border border-amber-100 text-amber-800 text-xs font-medium leading-relaxed">
            ...
            {files.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-2">
                {files.map((f, i) => (
                  <div key={i} className="flex items-center gap-2 bg-blue-50 border border-blue-100 text-blue-800 px-3 py-1.5 rounded-lg text-xs font-bold shadow-sm">
            ...
            <textarea
              ...
              className="flex-grow w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-5 text-sm outline-none focus:border-[#001D4A] transition-colors resize-none font-medium text-slate-700"
            ></textarea>
          </div>
        </div>

        {/* COLUMNA DERECHA: CHAT */}
        <div className="w-full lg:w-2/3 flex flex-col bg-white rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden">
          <div className="bg-slate-100 p-6 border-b border-slate-200 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-black text-slate-800 tracking-tight">Chat Interactivo</h3>
                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">Soporta: Español, Inglés, etc.</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex-grow overflow-y-auto p-6 space-y-6 bg-slate-50/50">
            {messages.map(msg => (
              <div key={msg.id} className={`flex gap-4 ${msg.sender === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0 mt-1 shadow-md ${msg.sender === 'user' ? 'bg-blue-600 text-white' : 'bg-[#001D4A] text-white'}`}>
                  {msg.sender === 'user' ? <User size={18} /> : <Bot size={18} />}
                </div>
                <div className={`px-5 py-4 rounded-[1.5rem] max-w-[80%] text-sm leading-relaxed shadow-sm ${msg.sender === 'user'
            ...
            {isLoading && (
              <div className="flex gap-4">
                <div className="w-10 h-10 rounded-2xl bg-[#001D4A] text-white flex items-center justify-center flex-shrink-0 mt-1 shadow-md">
                  <Bot size={18} />
                </div>
                <div className="px-5 py-4 rounded-[1.5rem] bg-white border border-slate-100 rounded-tl-sm flex items-center gap-3 shadow-sm">
                  <Loader2 size={18} className="text-[#001D4A] animate-spin" />
                  <span className="text-xs text-slate-500 font-bold tracking-wider uppercase">Analizando documentos...</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="p-5 bg-white border-t border-slate-100">
            <form onSubmit={handleSend} className="relative flex items-center">
              <input
                ...
                className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl pl-6 pr-16 py-4 text-sm outline-none focus:border-[#001D4A] transition-all font-medium text-slate-700"
                disabled={isLoading}
              />
              <button
                type="submit"
                disabled={!input.trim() || isLoading}
                className="absolute right-3 w-12 h-12 bg-[#001D4A] hover:bg-blue-800 disabled:opacity-50 text-white rounded-[1rem] flex items-center justify-center transition-all shadow-lg active:scale-95"
              >
