  ...
  return (
    <section>
      <div className="flex items-center gap-4 mb-8">
        ...
        <h2 className="text-4xl font-black text-slate-900 dark:text-white tracking-tighter transition-colors">Diagramas del Sistema</h2>
      </div>

      <div className="space-y-12">
        
        {/* Diagrama Arquitectónico */}
        <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-8 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 border border-slate-100 dark:border-slate-700 cursor-default">
          <h3 className="text-2xl font-black text-[#002B7F] dark:text-blue-400 mb-2 transition-colors">1. Arquitectura de Integración</h3>
          <p className="text-slate-600 dark:text-slate-400 mb-8 font-medium transition-colors">Cómo interactúa el Frontend (React) con los servicios BaaS (Supabase) y la Inteligencia Artificial (Google Gemini).</p>
          <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-700 overflow-hidden transition-colors">
        ...
        {/* Diagrama de Flujo */}
        <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-8 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 border border-slate-100 dark:border-slate-700 cursor-default">
          <h3 className="text-2xl font-black text-[#002B7F] dark:text-blue-400 mb-2 transition-colors">2. Flujo de Usuario (User Journey)</h3>
          <p className="text-slate-600 dark:text-slate-400 mb-8 font-medium transition-colors">La ruta de navegación desde la autenticación hasta los módulos interactivos clave.</p>
          <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-700 overflow-hidden transition-colors">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-8 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 border border-slate-100 dark:border-slate-700 cursor-default">
          <h3 className="text-2xl font-black text-[#002B7F] dark:text-blue-400 mb-2 transition-colors">3. Diagrama Entidad-Relación (ERD)</h3>
          <p className="text-slate-600 dark:text-slate-400 mb-8 font-medium transition-colors">Esquema relacional implementado en Supabase PostgreSQL.</p>
          <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-700 overflow-hidden transition-colors">
