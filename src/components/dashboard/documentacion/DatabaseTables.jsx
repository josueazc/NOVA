...
const TableDefinition = ({ tableName, description, columns }) => (
 <div className="bg-surface rounded-2xl p-8 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 border border-line mb-8 cursor-default">
  <div className="mb-6">
   <h3 className="text-2xl font-black text-accent mb-2 transition-colors inline-flex items-center gap-2">
    <Server size={20} /> {tableName}
   </h3>
   <p className="text-muted font-medium transition-colors">{description}</p>
  </div>
  
  <div className="overflow-x-auto rounded-2xl border border-line">
   <table className="w-full text-left text-sm">
    <thead className="bg-surface-2/50 text-muted uppercase font-black tracking-widest text-[10px]">
     <tr>
    ...
    <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50 text-muted font-medium">
     {columns.map((col, idx) => (
      <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
       <td className="px-6 py-4 font-bold text-ink">{col.name}</td>
       <td className="px-6 py-4"><span className="bg-blue-50 text-blue-600 px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-widest">{col.type}</span></td>
 ...
 return (
  <section>
   <div className="flex items-center gap-4 mb-8">
    ...
    <h2 className="text-4xl font-black text-ink tracking-tighter transition-colors">Diccionario de Datos</h2>
   </div>
