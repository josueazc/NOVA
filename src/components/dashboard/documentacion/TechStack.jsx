...
const TechColumn = ({ title, items }) => (
 <div className="hover:-translate-y-2 transition-transform duration-300">
  <h4 className="text-xs font-black text-faint uppercase tracking-widest mb-6">{title}</h4>
  <div className="space-y-6">
   {items.map((item, idx) => (
    <div key={idx} className="flex gap-4">
     <div className="shrink-0 mt-0.5">{item.icon}</div>
     <div>
      <p className="font-bold text-ink transition-colors leading-none">{item.name}</p>
      <p className="text-sm text-muted transition-colors mt-2">{item.desc}</p>
     </div>
    </div>
   ))}
  </div>
 ...
 return (
  <section>
   <div className="flex items-center gap-4 mb-8">
    <div className="w-12 h-12 bg-danger text-white rounded-2xl flex items-center justify-center shadow-lg transition-colors"><Microchip size={24} /></div>
    <h2 className="text-4xl font-black text-ink tracking-tighter transition-colors">Stack Tecnológico</h2>
   </div>
   
   <div className="bg-surface rounded-2xl p-8 md:p-10 shadow-lg hover:shadow-2xl transition-all duration-300 border border-line">
    <div className="grid md:grid-cols-3 gap-10">
     ...
     <TechColumn 
      ...
      items={[
       ...
       { name: "Prompt Engineering", desc: "Instrucciones de sistema estrictas (System Prompts).", icon: <Terminal className="text-muted w-5 h-5" /> }
      ]}
     />
    </div>
   </div>
