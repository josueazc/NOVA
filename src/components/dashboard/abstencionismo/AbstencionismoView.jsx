  ...
  <div className="w-full font-sans text-ink pb-24 pt-12 flex-grow animate-in fade-in slide-in-from-bottom-4 duration-500 transition-colors">
   <main className="max-w-[1600px] mx-auto px-6 sm:px-10 lg:px-24">
    ...
    <section className="mb-24">
     <div className="flex flex-col lg:flex-row gap-16 items-start">
      <div className="w-full lg:w-1/2 space-y-8">
       <div className="inline-flex items-center gap-2 bg-red-50 dark:bg-red-900/20 text-danger px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest border border-red-100 dark:border-red-900/30 shadow-sm">
        <span className="relative flex h-2 w-2"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-danger opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-danger"></span></span>
        Análisis Electoral
       </div>
       <h1 className="text-5xl sm:text-6xl lg:text-7xl font-black tracking-tighter leading-[0.85] text-ink">
        Abstencionismo<br />
        <span className="text-danger">Electoral</span>
       </h1>
       <div className="w-24 h-2 bg-danger rounded-full"></div>
       <p className="text-muted text-xl font-medium leading-relaxed max-w-lg">
        El abstencionismo es la <strong className="text-ink">no participación</strong> de los ciudadanos en el ejercicio de su derecho al voto. En Costa Rica, esta problemática ha alcanzado niveles históricos, amenazando la legitimidad democrática del país.
       </p>
      </div>
      ...
      <div className="w-full lg:w-1/2 grid grid-cols-2 gap-5">
       {[
        ...
        { label: 'Votos No Emitidos', value: '1.43M', icon: <UserX size={20} />, color: '#002B7F', bg: 'bg-blue-50 ' },
        ...
        { label: 'Padrón Electoral', value: '3.54M', icon: <Users size={20} />, color: '#002B7F', bg: 'bg-blue-50 ' },
       ].map((s, i) => (
    ...
    <RevealOnScroll>
     <section className="mb-24" ref={chartRef}>
      <div className="mb-12 space-y-3">
       <span className="text-[10px] font-black uppercase tracking-[0.3em] text-danger">Serie Histórica</span>
       <h2 className="text-4xl sm:text-5xl font-black text-accent tracking-tighter">Evolución del Abstencionismo</h2>
       <p className="text-muted text-lg font-medium max-w-2xl">Primera ronda de elecciones presidenciales (1998–2022). Datos oficiales del TSE.</p>
      </div>
      <div className="bg-surface rounded-2xl p-8 sm:p-12 shadow-sm border border-line overflow-hidden">
        ...
        {datosRecientes.map((d, i) => {
         ...
         return (
          <div key={d.año} className="flex-1 flex flex-col items-center justify-end h-full group cursor-pointer">
           <div className="text-[10px] sm:text-xs font-black text-muted mb-2 opacity-0 group-hover:opacity-100 transition-opacity">{d.abstencionismo}%</div>
           <div
           ...
           ></div>
           <div className="mt-3 text-[10px] sm:text-xs font-black text-faint">{d.año}</div>
          </div>
         );
        })}
       </div>
    ...
    <RevealOnScroll>
     <section className="mb-24">
      <div className="mb-12 space-y-3">
       <span className="text-[10px] font-black uppercase tracking-[0.3em] text-danger">Diagnóstico</span>
       <h2 className="text-4xl sm:text-5xl font-black text-accent tracking-tighter">¿Por qué no votan?</h2>
       <p className="text-muted text-lg font-medium max-w-2xl">Principales razones identificadas por el TSE y el CIEP-UCR.</p>
      </div>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
       {razonesAbstencionismo.map((r) => (
        <div key={r.id} className="bg-surface rounded-2xl p-8 shadow-sm border border-line hover:shadow-xl transition-all group relative overflow-hidden">
         ...
         <div className="relative z-10">
          ...
          <div className="text-3xl font-black tracking-tighter mb-2" style={{ color: r.color }}>{r.porcentaje}%</div>
          <h3 className="text-lg font-black text-ink tracking-tight mb-3">{r.razon}</h3>
          <p className="text-sm text-muted font-medium leading-relaxed">{r.descripcion}</p>
         </div>
        </div>
       ))}
      </div>
    ...
    <RevealOnScroll>
     <section className="mb-24">
      <div className="mb-12 space-y-3">
       <span className="text-[10px] font-black uppercase tracking-[0.3em] text-danger">Geografía Electoral</span>
       <h2 className="text-4xl sm:text-5xl font-black text-accent tracking-tighter">Abstención por Provincia</h2>
       <p className="text-muted text-lg font-medium max-w-2xl">Resultados de la Primera Ronda Electoral 2022 por provincia.</p>
      </div>
      <div className="bg-surface rounded-2xl p-8 sm:p-12 shadow-sm border border-line">
         ...
         <div key={nombre} className="group">
          <div className="flex flex-col sm:flex-row justify-between sm:items-end mb-2 gap-2">
           <div className="flex-1">
            <span className="text-xl font-black text-ink tracking-tight block mb-1">{nombre}</span>
            <span className="text-sm font-medium text-muted block max-w-xl leading-relaxed">{d.descripcion}</span>
           </div>
           <span className="text-3xl font-black tracking-tighter shrink-0" style={{ color: d.color }}>{d.abstencionismo}%</span>
          </div>
          <div className="w-full bg-surface-2 rounded-full h-4 overflow-hidden shadow-inner mt-3">
    ...
    <RevealOnScroll>
     <section className="mb-24">
      <div className="mb-12 space-y-3">
       <span className="text-[10px] font-black uppercase tracking-[0.3em] text-danger">Análisis Demográfico</span>
       <h2 className="text-4xl sm:text-5xl font-black text-accent tracking-tighter">¿Quiénes no votan?</h2>
      </div>
      <div className="grid lg:grid-cols-3 gap-8">
       {/* Por Edad */}
       <div className="bg-surface rounded-2xl p-8 shadow-sm border border-line lg:col-span-2">
        <h3 className="text-lg font-black text-accent tracking-tighter mb-8">Abstención por Grupo de Edad</h3>
        <div className="space-y-5">
         {datosDemograficos.porEdad.map((g) => (
          <div key={g.grupo} className="group">
           <div className="flex justify-between mb-1.5">
            <span className="text-sm font-bold text-muted">{g.grupo} años <span className="text-faint text-xs">({g.label})</span></span>
            <span className="text-sm font-black tracking-tighter" style={{ color: g.abstencionismo > 40 ? '#DC2626' : g.abstencionismo > 35 ? '#F97316' : '#22C55E' }}>{g.abstencionismo}%</span>
           </div>
       ...
       <div className="space-y-6">
        <div className="bg-accent rounded-2xl p-8 text-white relative overflow-hidden">
         ...
         <div className="relative z-10">
          ...
          <div className="space-y-4">
           <div>
            ...
            <div className="text-4xl font-black tracking-tighter">{datosDemograficos.brechaUrbanoRural.urbano}%</div>
           </div>
           <div>
            <div className="text-blue-200 text-xs font-bold mb-1">Rural</div>
            <div className="text-4xl font-black tracking-tighter text-danger">{datosDemograficos.brechaUrbanoRural.rural}%</div>
           </div>
           <div className="pt-4 border-t border-white/10">
            <div className="text-blue-200 text-xs font-bold">Diferencia</div>
            <div className="text-2xl font-black tracking-tighter text-[#FBBF24]">+{datosDemograficos.brechaUrbanoRural.diferencia}%</div>
           </div>
          </div>
         </div>
        </div>
        ...
        <div className="bg-surface rounded-2xl p-8 shadow-sm border border-line">
         ...
         {comparativaMunicipalNacional.map(c => (
          <div key={c.tipo} className="flex justify-between items-center py-2 border-b border-line last:border-0">
           ...
           <span className="text-sm font-black tracking-tighter" style={{ color: c.abstencionismo > 60 ? '#DC2626' : c.abstencionismo > 35 ? '#F97316' : '#22C55E' }}>{c.abstencionismo}%</span>
          </div>
         ))}
        </div>
       </div>
    ...
    <RevealOnScroll>
     <section className="mb-12">
      <div className="mb-12 space-y-3">
       <span className="text-[10px] font-black uppercase tracking-[0.3em] text-danger">Soluciones</span>
       <h2 className="text-4xl sm:text-5xl font-black text-accent tracking-tighter">¿Cómo revertirlo?</h2>
       <p className="text-muted text-lg font-medium max-w-2xl">Estrategias basadas en estudios del TSE, CIEP-UCR y experiencias internacionales.</p>
      </div>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
       {estrategiasPrevencion.map((e) => (
        <div key={e.id} className="bg-surface rounded-2xl p-8 shadow-sm border border-line hover:shadow-xl hover:-translate-y-2 transition-all group relative overflow-hidden">
         ...
         <div className="relative z-10">
          <div className="w-14 h-14 rounded-2xl bg-blue-50 text-accent flex items-center justify-center mb-6 shadow-lg group-hover:rotate-6 transition-transform">{getIcon(e.icono, 28)}</div>
          <h3 className="text-xl font-black text-ink tracking-tighter mb-3">{e.titulo}</h3>
      ...
      {/* CTA Final */}
      <div className="mt-16 bg-gradient-to-br from-[#002B7F] to-[#001A4A] rounded-3xl p-10 sm:p-14 text-white relative overflow-hidden">
       ...
       <div className="relative z-10 max-w-2xl">
        ...
        <h3 className="text-3xl sm:text-4xl font-black leading-tight tracking-tighter mb-4">Informarte es el primer paso para participar.</h3>
