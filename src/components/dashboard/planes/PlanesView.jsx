 ...
 return (
  <div className="w-full pb-20 animate-in fade-in zoom-in-95 duration-500 mt-10">
   <main className="container mx-auto px-6 max-w-6xl">
    ...
    <div className="bg-surface rounded-3xl shadow-xl border border-line p-8 md:p-12 mb-12 relative overflow-visible transition-all duration-500">
     <div className="grid md:grid-cols-2 gap-12 items-start relative z-10">
      <div className="space-y-6">
       <div className="flex flex-wrap items-center justify-between gap-4">
        <h2 className="text-4xl font-black tracking-tighter leading-tight transition-colors">
         Analizá el <span style={{ color: accentColor }}>Plan de Gobierno</span>
        </h2>
        {selectedParty && (
         <div className="flex items-center gap-3 bg-surface-2 px-4 py-2 rounded-full shadow-sm border border-line animate-in fade-in zoom-in transition-colors">
           ...
           <span className="text-[10px] font-black uppercase tracking-widest text-muted transition-colors">{selectedParty.name}</span>
         </div>
        )}
       </div>
       ...
       <div className="relative" ref={dropdownRef}>
        ...
        {/* Lista Desplegable */}
        {isDropdownOpen && (
         <div className="absolute top-full left-0 right-0 mt-2 bg-surface rounded-3xl shadow-2xl border border-line z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 transition-colors">
      ...
      <div className="bg-surface-2 rounded-2xl p-8 space-y-6 flex flex-col justify-center h-full transition-colors">
       <div className="flex items-start gap-4">
        ...
        <div>
         <p className="text-sm font-bold text-muted leading-tight transition-colors">Procesaremos el texto del documento para encontrar propuestas reales.</p>
         <p className="text-[10px] text-faint font-bold mt-2 uppercase tracking-widest transition-colors">Basado en Inteligencia Artificial</p>
        </div>
       </div>
    ...
    <div className="mb-12">
     <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {areasTematicas.map((area) => (
       ...
       >
        <div className={`p-4 rounded-2xl ${selectedArea === area.id ? 'bg-white/20' : 'bg-surface-2 transition-colors'}`}>
        ...
        <div className="text-center">
         ...
         {analysisResult && (
          <span className={`text-[9px] font-bold ${selectedArea === area.id ? 'text-white/60' : 'text-faint transition-colors'}`}>
    ...
    <div ref={resultsRef} className="min-h-[200px]">
     {analysisResult && selectedArea ? (
      <div className="bg-surface rounded-[4rem] shadow-2xl border border-line overflow-hidden animate-in slide-in-from-bottom-10 transition-colors">
       <div className="p-8 md:p-12 border-b border-line/50 flex items-center justify-between transition-colors" style={{ backgroundColor: lightAccent }}>
        <div className="flex items-center gap-6">
         <div className="w-16 h-16 rounded-2xl text-white flex items-center justify-center shadow-lg" style={{ backgroundColor: accentColor }}>
        ...
        {analysisResult[selectedArea]?.length > 0 ? (
         analysisResult[selectedArea].map((prop, i) => (
          <div key={i} className="flex gap-6 p-6 rounded-2xl bg-surface-2/30 hover:bg-white border-2 border-transparent hover:border-slate-100 dark:hover:border-slate-600 transition-all group">
           ...
           <p className="font-bold text-ink text-lg leading-relaxed transition-colors">{prop}</p>
          </div>
         ))
        ) : (
